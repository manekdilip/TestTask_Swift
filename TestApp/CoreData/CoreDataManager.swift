...
class CoreDataManager : NSObject{
    ...
    @objc func createObjectForEntity(entityName:String,taskContext: NSManagedObjectContext) -> AnyObject?{
        if (entityName != "")
        {
    ...
    func deleteAllData(_ strEntityName:String) {
        let fetch = NSFetchRequest<NSFetchRequestResult>(entityName: strEntityName)
        let request = NSBatchDeleteRequest(fetchRequest: fetch)
        do {
            _ = try self.bGManagedObjectContext.execute(request)
        } catch {
